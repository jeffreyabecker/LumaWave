...
public:
    ...
    ProtocolTransportPipeline(std::unique_ptr<protocols::IProtocol> protocol,
                              std::unique_ptr<transports::ITransport> transport)
        : _protocol(std::move(protocol)), _transport(std::move(transport))
    {
    }
    ...
    void write(span<const lw::colors::Color> colors, BrightnessType brightness) override
    {
        ...
        if (applyBrightness)
        {
            ...
            for (size_t i = 0; i < colors.size(); ++i)
            {
                ...
                {
                    dst[channel] = static_cast<lw::colors::ColorComponent>(
                        lw::colors::applyBrightness(colors[i][channel], brightness));
                }
            }
