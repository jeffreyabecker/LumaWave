...
class RpPioTransport : public ITransport
{
  public:
    ...
    void begin() override
    {
        ...
        const uint clockPin =
            clockMapped ? static_cast<uint>(_config.clockPin) : static_cast<uint>(_config.dataPin + 1);

        _smLease =
            RpPioManager::requestStateMachine(&Apa102MiniProgram::Program, static_cast<int8_t>(_config.pioIndex));
        if (!_smLease.isValid())
        {
            return;
        }
