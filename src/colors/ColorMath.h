...
struct ScalarColorMathBackend
{
  ...
  static constexpr uint8_t linearBlendScalar8(uint8_t left, uint8_t right, uint8_t progress)
  {
    ...
    const uint32_t numerator = (static_cast<uint32_t>(left) * inverseProgress) + (static_cast<uint32_t>(right) * static_cast<uint32_t>(progress)) + 127u;
    return static_cast<uint8_t>(numerator / 255u);
  }

  static constexpr uint8_t bilinearBlendScalar8(uint8_t c00, uint8_t c01, uint8_t c10, uint8_t c11, uint8_t x, uint8_t y)
  {
  ...
  static constexpr void hsbToRgb8(uint8_t hue, uint8_t saturation, uint8_t brightness, uint8_t& r, uint8_t& g, uint8_t& b)
  {
    if (saturation == 0u)
    {
      r = brightness;
  ...
  static constexpr void hslToRgb8(uint8_t hue, uint8_t saturation, uint8_t lightness, uint8_t& r, uint8_t& g, uint8_t& b)
  {
    if (saturation == 0u)
    {
      r = lightness;
    ...
    const int16_t q = (lightness < 128u) ? static_cast<int16_t>((static_cast<uint16_t>(lightness) * (255u + saturation) + 127u) / 255u)
                                         : static_cast<int16_t>(lightness + saturation - ((static_cast<uint16_t>(lightness) * saturation + 127u) / 255u));
...
template <typename TValue, typename TBrightness,
          typename = std::enable_if_t<std::is_integral_v<TValue> && std::is_unsigned_v<TValue> && std::is_integral_v<TBrightness> && std::is_unsigned_v<TBrightness> && !std::is_same_v<std::remove_cv_t<TValue>, bool> &&
...
template <typename TTarget, typename TSource,
          typename = std::enable_if_t<std::is_integral_v<TTarget> && std::is_unsigned_v<TTarget> && std::is_integral_v<TSource> && std::is_unsigned_v<TSource> && !std::is_same_v<std::remove_cv_t<TTarget>, bool> &&
...
template <typename TTarget, typename TValue, typename TProgress, typename TProgressMax,
          typename = std::enable_if_t<std::is_integral_v<TTarget> && std::is_unsigned_v<TTarget> && std::is_integral_v<TValue> && std::is_unsigned_v<TValue> && std::is_integral_v<TProgress> && std::is_unsigned_v<TProgress> &&
                                      std::is_integral_v<TProgressMax> && std::is_unsigned_v<TProgressMax> && !std::is_same_v<std::remove_cv_t<TTarget>, bool> && !std::is_same_v<std::remove_cv_t<TValue>, bool> &&
