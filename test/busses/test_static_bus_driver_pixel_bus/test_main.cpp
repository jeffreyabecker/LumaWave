...
public:
  ...
  explicit MockTransport(TransportSettingsType settings) : _settings(settings) {}
...
void test_constructor_manages_internal_typed_buffers_and_runs_pipeline(void)
{
  ...
  TEST_ASSERT_EQUAL_UINT32(static_cast<uint32_t>(protocolBytes.size()), static_cast<uint32_t>(bus.protocol().lastBufferSize));

  TEST_ASSERT_EQUAL_UINT32(static_cast<uint32_t>(protocolBytes.size()), static_cast<uint32_t>(bus.transport().transmitted.size()));
...
void test_platform_default_transport_template_default_constructs(void)
{
  ...
  TEST_ASSERT_EQUAL_UINT32(static_cast<uint32_t>(bus.protocolBuffer().size()), static_cast<uint32_t>(bus.protocol().lastBufferSize));
}
...
void test_pixel_bus_scales_before_protocol_and_marks_transport_context(void)
{
  ...
  TEST_ASSERT_FALSE(bus.transport().lastBrightness.upstreamApplied);
}
} // namespace
