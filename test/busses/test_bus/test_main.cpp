...
public:
  ...
  void write(lw::span<const lw::Pixel> colors) override
  {
    ++writeCount;
    if (!colors.empty())
    {
...
void test_bus_single_run_writes_on_show(void)
{
  ...
  lw::buses::Bus bus(lw::span<lw::Pixel>{&pixel, 1}, lw::span<const lw::buses::PipelineRun>{runs});
  ...
  TEST_ASSERT_EQUAL_UINT8(10, lw::pixelR(mock.lastColor));
  TEST_ASSERT_EQUAL_UINT8(20, lw::pixelG(mock.lastColor));
  TEST_ASSERT_EQUAL_UINT8(30, lw::pixelB(mock.lastColor));
...
void test_bus_respects_pipeline_readiness(void)
{
  ...
  bus.pixels()[0] = lw::pixelFromRGB(5, 6, 7);
  bus.show();
...
{
  std::array<lw::Pixel, 5> buf{};
  MockPipeline mock1;
  MockPipeline mock2;

  lw::buses::PipelineRun runs[] = {{&mock1, 2}, {&mock2, 3}};
  lw::buses::Bus bus(lw::span<lw::Pixel>{buf}, lw::span<const lw::buses::PipelineRun>{runs});

  buf[0] = lw::pixelFromRGB(1, 0, 0);
  buf[1] = lw::pixelFromRGB(2, 0, 0);
  buf[2] = lw::pixelFromRGB(3, 0, 0);
  buf[3] = lw::pixelFromRGB(4, 0, 0);
  buf[4] = lw::pixelFromRGB(5, 0, 0);
  ...
  TEST_ASSERT_EQUAL_UINT8(1, lw::pixelR(mock1.lastColor));
  TEST_ASSERT_EQUAL_UINT8(3, lw::pixelR(mock2.lastColor));
}

void test_bus_writes_go_to_caller_buffer(void)
{
  std::array<lw::Pixel, 3> buf{};
  MockPipeline mock;
