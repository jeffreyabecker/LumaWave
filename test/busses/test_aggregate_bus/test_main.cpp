...
public:
  ...
  explicit StubBus(lw::span<lw::Color> pixels, bool ready = true) : _pixels(pixels), _ready(ready) {}
...
void test_reference_aggregate_bus_pixels_concatenate_child_views(void)
{
  ...
  lw::busses::ReferenceAggregateBus aggregate(lw::span<lw::IPixelBus*>{buses.data(), buses.size()});
