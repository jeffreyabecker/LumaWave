...
void test_palette_first_pass_compile(void)
{
  ...
  static_assert(lw::palettes::IsPaletteLike<lw::palettes::Palette>::value, "Palette<Color> must satisfy IsPaletteLike");
  static_assert(lw::palettes::Palette::AllowedSettings.size() == 0u, "Palette<Color> must expose an empty AllowedSettings descriptor");
  ...
  std::array<lw::palettes::PaletteStop, 2> sampleStops = {lw::palettes::PaletteStop{0, lw::colorFromRGB(0, 0, 0)}, lw::palettes::PaletteStop{255, lw::colorFromRGB(255, 255, 255)}};
  ...
  const size_t sampledCount = lw::palettes::samplePalette(samplePaletteLike, sampleIndexes, lw::span<lw::Color>(sampledOutput.data(), sampledOutput.size()), options);
  TEST_ASSERT_EQUAL_UINT32(static_cast<uint32_t>(sampledOutput.size()), static_cast<uint32_t>(sampledCount));

  const size_t ownedSampledCount = lw::palettes::samplePalette(ownedPaletteLike, sampleIndexes, lw::span<lw::Color>(sampledOutput.data(), sampledOutput.size()), options);
  ...
  const size_t nearestSampledCount = lw::palettes::samplePalette(samplePaletteLike, nearestSampleIndexes, lw::span<lw::Color>(sampledOutput.data(), sampledOutput.size()), nearestOptions);
  ...
  const size_t midpointSampledCount = lw::palettes::samplePalette(samplePaletteLike, midpointIndexes, lw::span<lw::Color>(sampledOutput.data(), sampledOutput.size()), midpointOptions);
