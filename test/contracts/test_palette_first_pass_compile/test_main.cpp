...
void test_palette_first_pass_compile(void)
{
    ...
    static_assert(lw::colors::palettes::IsPaletteLike<lw::colors::palettes::Palette>::value,
                  "Palette<Color> must satisfy IsPaletteLike");
    static_assert(lw::colors::palettes::Palette::AllowedSettings.size() == 0u,
                  "Palette<Color> must expose an empty AllowedSettings descriptor");
    ...
    lw::colors::palettes::Palette invalidPalette(
        lw::span<const lw::colors::palettes::PaletteStop>(&invalidStop, 1));
    TEST_ASSERT_TRUE(invalidPalette.stops().empty());

    std::array<lw::colors::palettes::PaletteStop, 2> sampleStops = {
        lw::colors::palettes::PaletteStop{0, lw::Color(0, 0, 0)},
        lw::colors::palettes::PaletteStop{255, lw::Color(255, 255, 255)}};
    lw::colors::palettes::Palette samplePaletteLike(
        lw::span<const lw::colors::palettes::PaletteStop>(sampleStops.data(), sampleStops.size()));
    lw::colors::palettes::Palette ownedPaletteLike(
        std::vector<lw::colors::palettes::PaletteStop>(sampleStops.begin(), sampleStops.end()));
    ...
    const size_t sampledCount = lw::colors::palettes::samplePalette(
        samplePaletteLike, sampleIndexes, lw::span<lw::Color>(sampledOutput.data(), sampledOutput.size()), options);
    ...
    const size_t ownedSampledCount = lw::colors::palettes::samplePalette(
        ownedPaletteLike, sampleIndexes, lw::span<lw::Color>(sampledOutput.data(), sampledOutput.size()), options);
    ...
    const size_t nearestSampledCount = lw::colors::palettes::samplePalette(
        samplePaletteLike, nearestSampleIndexes, lw::span<lw::Color>(sampledOutput.data(), sampledOutput.size()),
        nearestOptions);
    ...
    const size_t midpointSampledCount = lw::colors::palettes::samplePalette(
        samplePaletteLike, midpointIndexes, lw::span<lw::Color>(sampledOutput.data(), sampledOutput.size()),
        midpointOptions);
