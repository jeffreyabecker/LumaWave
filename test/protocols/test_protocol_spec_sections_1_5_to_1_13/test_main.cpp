...
void test_1_5_1_lpd6803_packed_5_5_5_serialization(void)
{
  ...
  protocol.begin();
  protocol.update(std::array<lw::Rgbw8Color, 1>{lw::Rgbw8Color{0xFF, 0x00, 0x88}}, as_span(protocolBuffer));
...
void test_1_5_2_lpd6803_framing_end_frame_size(void)
{
  ...
  for (uint16_t n : counts)
  {
    ...
    auto protocolBuffer = bind_protocol_buffer(protocol);
    std::vector<lw::Rgbw8Color> colors(n, lw::Rgbw8Color{1, 2, 3});
    protocol.begin();
    protocol.update(lw::span<const lw::Rgbw8Color>{colors.data(), colors.size()}, as_span(protocolBuffer));
...
void test_1_6_1_lpd8806_7bit_plus_msb_serialization(void)
{
  ...
  auto protocolBuffer = bind_protocol_buffer(protocol);
  protocol.begin();
  protocol.update(std::array<lw::Rgbw8Color, 1>{lw::Rgbw8Color{0x00, 0xFF, 0x80}}, as_span(protocolBuffer));
...
void test_1_7_1_and_1_7_2_p9813_header_checksum_and_framing(void)
{
  ...
  auto protocolBuffer = bind_protocol_buffer(protocol);
  protocol.begin();
  protocol.update(std::array<lw::Rgbw8Color, 1>{lw::Rgbw8Color{0x80, 0x40, 0x00}}, as_span(protocolBuffer));
...
void test_1_8_1_sm168x_variant_resolution_and_frame_sizing(void)
{
  auto run_case = [&](size_t expectedFrameSize, auto protocolFactory, const char* order)
  {
    ...
    auto protocolBuffer = bind_protocol_buffer(protocol);
    std::vector<lw::Rgbw8Color> colors(2, lw::Rgbw8Color{1, 2, 3, 4});
    protocol.update(lw::span<const lw::Rgbw8Color>{colors.data(), colors.size()}, as_span(protocolBuffer));

    TEST_ASSERT_EQUAL_UINT32(expectedFrameSize, static_cast<uint32_t>(protocolBuffer.size()));
  };

  run_case(10U, [](lw::protocols::Sm168xProtocolSettings settings) { return lw::protocols::Sm168xProtocol<lw::Rgbw8Color>(2, std::move(settings)); }, "RGBW");
  run_case(10U, [](lw::protocols::Sm168xProtocolSettings settings) { return lw::protocols::Sm168xProtocol<lw::Rgbw8Color>(2, std::move(settings)); }, "RGBW");
}
...
{
  {
    ...
    lw::protocols::Sm168xProtocol<lw::Rgbw8Color> protocol(1, std::move(settings));
    auto protocolBuffer = bind_protocol_buffer(protocol);
...
void test_1_9_1_sm16716_buffer_size_and_start_bit_prefix(void)
{
  ...
  auto protocolBuffer = bind_protocol_buffer(protocol);
  protocol.update(std::array<lw::Rgbw8Color, 1>{lw::Rgbw8Color{0, 0, 0}}, as_span(protocolBuffer));
