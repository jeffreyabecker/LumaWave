...
std::vector<uint8_t> encode_onewire_payload(const std::vector<uint8_t>& raw,
                                            const lw::transports::OneWireTiming& timing, bool protocolIdleHigh = false,
                                            uint8_t prefixResetMultiplier = 1, uint8_t suffixResetMultiplier = 1)
{
    std::vector<uint8_t> source = raw;
    const size_t payloadBytes =
        lw::transports::OneWireEncoding::expandedPayloadSizeBytes(raw.size(), timing.bitPattern());
    const size_t prefixResetBytes =
        lw::transports::OneWireEncoding::computeResetBytes(timing, 0, prefixResetMultiplier);
    const size_t suffixResetBytes =
        lw::transports::OneWireEncoding::computeResetBytes(timing, 0, suffixResetMultiplier);
    std::vector<uint8_t> encoded(payloadBytes + prefixResetBytes + suffixResetBytes, 0);
    const size_t encodedSize = lw::transports::OneWireEncoding::encodeWithResets(
        source.data(), source.size(), encoded.data(), encoded.size(), timing, 0, prefixResetMultiplier,
        suffixResetMultiplier, protocolIdleHigh);
...
{
    lw::protocols::Lpd6803ProtocolT<> protocol(
        1, lw::protocols::Lpd6803ProtocolSettings{{}, lw::ChannelOrder::RGB::value});
...
void test_1_5_2_lpd6803_framing_end_frame_size(void)
{
    ...
    {
        lw::protocols::Lpd6803ProtocolT<> protocol(
            n, lw::protocols::Lpd6803ProtocolSettings{{}, lw::ChannelOrder::RGB::value});
        auto protocolBuffer = bind_protocol_buffer(protocol);
...
{
    lw::protocols::Lpd8806ProtocolT<> protocol(
        1, lw::protocols::Lpd8806ProtocolSettings{{}, lw::ChannelOrder::RGB::value});
    auto protocolBuffer = bind_protocol_buffer(protocol);
    protocol.begin();
...
void test_1_6_2_lpd8806_symmetric_start_end_framing(void)
{
    ...
    {
        lw::protocols::Lpd8806ProtocolT<> protocol(
            n, lw::protocols::Lpd8806ProtocolSettings{{}, lw::ChannelOrder::RGB::value});
        auto protocolBuffer = bind_protocol_buffer(protocol);
        std::vector<lw::Rgb8Color> colors(n, lw::Rgb8Color{1, 2, 3});
        protocol.begin();
        protocol.update(lw::span<const lw::Rgb8Color>{colors.data(), colors.size()}, as_span(protocolBuffer));
...
void test_1_7_1_and_1_7_2_p9813_header_checksum_and_framing(void)
{
    ...
    const uint8_t expectedHeader = static_cast<uint8_t>(0xC0 | (((~0x00u >> 6) & 0x03) << 4) |
                                                        (((~0x40u >> 6) & 0x03) << 2) | ((~0x80u >> 6) & 0x03));
...
void test_1_8_1_sm168x_variant_resolution_and_frame_sizing(void)
{
    ...
    run_case(8U, [](lw::protocols::Sm168xProtocolSettings settings)
             { return lw::protocols::Sm168xProtocol<lw::Rgbcw8Color, lw::Rgb8Color>(2, std::move(settings)); });
    run_case(10U, [](lw::protocols::Sm168xProtocolSettings settings)
             { return lw::protocols::Sm168xProtocol<lw::Rgbcw8Color, lw::Rgbw8Color>(2, std::move(settings)); });
    run_case(14U, [](lw::protocols::Sm168xProtocolSettings settings)
...
{
    lw::protocols::Sm16716ProtocolT<> protocol(
        1, lw::protocols::Sm16716ProtocolSettings{{}, lw::ChannelOrder::RGB::value});
    auto protocolBuffer = bind_protocol_buffer(protocol);
...
void test_1_12_1_1_12_2_1_12_3_tm1814_currents_inversion_and_payload_order(void)
{
    ...
    const std::vector<uint8_t> expectedRaw{63,
                                           0,
                                           25,
                                           63,
...
{
    {
        ...
        TEST_ASSERT_EQUAL_UINT32(
            static_cast<uint32_t>(
                encode_onewire_payload(std::vector<uint8_t>{63, 63, 63, 63, 192, 192, 192, 192, 4, 1, 2, 3},
                                       lw::transports::timing::Tm1814, true)
                    .size()),
            static_cast<uint32_t>(protocolBuffer.size()));
    }

    {
        ...
        TEST_ASSERT_EQUAL_UINT32(
            static_cast<uint32_t>(
                encode_onewire_payload(std::vector<uint8_t>{63, 63, 63, 63, 192, 192, 192, 192, 10, 9, 11, 0},
                                       lw::transports::timing::Tm1814, true)
                    .size()),
            static_cast<uint32_t>(protocolBuffer.size()));
    }
...
void test_1_13_1_and_1_13_2_tm1914_mode_matrix_inversion_and_payload_order(void)
{
    auto run_mode = [&](lw::protocols::Tm1914Mode mode, uint8_t expectedMode)
    {
        ...
        const std::vector<uint8_t> expectedRaw{0xFF,
                                               0xFF,
                                               expectedMode,
                                               static_cast<uint8_t>(~0xFF),
                                               static_cast<uint8_t>(~0xFF),
                                               static_cast<uint8_t>(~expectedMode),
                                               2,
                                               1,
                                               3};
...
{
    {
        ...
        TEST_ASSERT_EQUAL_UINT32(
            static_cast<uint32_t>(
                encode_onewire_payload(std::vector<uint8_t>{0xFF, 0xFF, 0xF5, 0x00, 0x00, 0x0A, 2, 1, 3},
                                       lw::transports::timing::Tm1914, true)
                    .size()),
            static_cast<uint32_t>(protocolBuffer.size()));
    }

    {
        ...
        TEST_ASSERT_EQUAL_UINT32(
            static_cast<uint32_t>(
                encode_onewire_payload(std::vector<uint8_t>{0xFF, 0xFF, 0xF5, 0x00, 0x00, 0x0A, 8, 7, 9},
                                       lw::transports::timing::Tm1914, true)
                    .size()),
            static_cast<uint32_t>(protocolBuffer.size()));
    }
