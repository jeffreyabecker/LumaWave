...
{
  const std::array<lw::Pixel, 2> colors{lw::pixelFromRGB(0x11, 0x22, 0x33), lw::pixelFromRGB(0x44, 0x55, 0x66)};
...

  {
    ...
    auto protocolBuffer = bind_protocol_buffer(protocol);
    protocol.begin();
    protocol.update(std::array<lw::Pixel, 1>{lw::pixelFromRGB(0x12, 0x34, 0x56)}, as_span(protocolBuffer));
...
{
  const std::array<lw::Pixel, 3> oversized{lw::pixelFromRGB(1, 2, 3), lw::pixelFromRGB(4, 5, 6), lw::pixelFromRGB(7, 8, 9)};
...

  {
    ...
    auto protocolBuffer = bind_protocol_buffer(protocol);
    protocol.begin();
    protocol.update(std::array<lw::Pixel, 2>{lw::pixelFromRGB(10, 11, 12), lw::pixelFromRGB(13, 14, 15)}, as_span(protocolBuffer));
    TEST_ASSERT_EQUAL_UINT32(8U, static_cast<uint32_t>(slice_bytes(protocolBuffer, 4, 8).size()));
  }
}
...
{
  const std::array<lw::Pixel, 2> colors{lw::pixelFromRGB(1, 2, 3), lw::pixelFromRGB(4, 5, 6)};
...
{
  const std::array<lw::Pixel, 1> colors8{lw::pixelFromRGB(0x11, 0x22, 0x33)};
...

  {
    ...
    protocol16.update(std::array<lw::Pixel, 1>{lw::pixelFromRGB(0x12, 0x34, 0x56)}, as_span(protocolBuffer));
...

  {
    ...
    auto protocolBuffer = bind_protocol_buffer(protocol16);
    protocol16.update(std::array<lw::Pixel, 1>{lw::pixelFromRGBW(0x01, 0x02, 0x03, 0x04)}, as_span(protocolBuffer));
...

  {
    ...
    protocolMixed.update(std::array<lw::Pixel, 1>{lw::pixelFromRGB(0x12, 0x34, 0x56)}, as_span(protocolBuffer));
    assert_bytes_equal(protocolBuffer, encode_ws2812x_payload(std::vector<uint8_t>{0x12, 0x34, 0x56}));
  }
...
{
  const lw::Pixel one = lw::pixelFromRGBW(1, 2, 3, 4);

  auto run_case = [&](const char* order, const std::vector<uint8_t>& expected)
  {
    lw::protocols::Ws2812xProtocol protocol(1, lw::protocols::Ws2812xProtocolSettings(order));
    auto protocolBuffer = bind_protocol_buffer(protocol);
    protocol.update(std::array<lw::Pixel, 1>{one}, as_span(protocolBuffer));
...
void test_1_14_4_ws2812x_readiness_wait_loop_contract(void)
{
  ...
  auto protocolBuffer = bind_protocol_buffer(protocol);
  protocol.update(std::array<lw::Pixel, 1>{lw::pixelFromRGB(9, 8, 7)}, as_span(protocolBuffer));
...
void test_1_14_5_ws2812x_oversized_span_contract(void)
{
  ...
  protocol.update(std::array<lw::Pixel, 3>{lw::pixelFromRGB(1, 2, 3), lw::pixelFromRGB(4, 5, 6), lw::pixelFromRGB(7, 8, 9)}, as_span(protocolBuffer));
